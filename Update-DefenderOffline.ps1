...
# Schema versions this script was built against. Bump when shipping a
# breaking config or hosts layout change. Test-SchemaVersion warns on
# mismatch but does not block startup.
...
# Schema version checks (config.conf + hosts.conf). Logs WARN on mismatch
# but doesn't gate execution — operators may legitimately run mixed-release
# scripts during a rolling upgrade.
$cfgSchema = Test-SchemaVersion -ConfigData $cfg `
    -ExpectedVersion $Script:ExpectedConfigSchemaVersion -ArtifactName 'config.conf'
if ($cfgSchema.Warning) { Write-Log $cfgSchema.Warning 'WARN' }
...
if ($hostsSchema.Warning) { Write-Log $hostsSchema.Warning 'WARN' }
$credParts = @()
